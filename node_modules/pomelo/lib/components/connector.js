...
var handleMessage = function(self, session, msg) {
  ...
  if (msg.body && msg.body.content) {
    try {
      var contentDetail = JSON.parse(msg.body.content);

      if (contentDetail.type == 'confirm' && contentDetail.interact == 'teacheraskexercises') {
        logCollector.info('%j', {
          'request_id': session.id,
          'event_user_id': msg.body.form,
          'event_name': 'tb_confirm',
          'event_info': {
            'message': msg.body.content,
            'sid': self.app.serverId,
            'rid': msg.body.rid
          }
        });
