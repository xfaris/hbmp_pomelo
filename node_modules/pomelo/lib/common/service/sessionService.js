...
//2017-09-04 用户重连时踢出
SessionService.prototype.duplicateLoginKick = function(uid, reason, cb) {
  // compatible for old kick(uid, cb);
  if (typeof reason === 'function') {
    cb = reason;
    reason = 'kick';
  ...
  if (sessions) {
    ...
    sids.forEach(function(sid) {
      self.sessions[sid].closed('DuplicateLoginKick');
    });
...
Session.prototype.closed = function(reason) {
  ...
  //2017-09-04 用户重复登录禁止触发事件
  //this.emit('closed', this.toFrontendSession(), reason);
  if (reason != 'DuplicateLoginKick') {
    this.emit('closed', this.toFrontendSession(), reason);
  }
